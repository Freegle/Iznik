...
try {
  ...
  assert(result.raw.startsWith('# Bug Reports — Live Summary'), 'has title')
  ...
  assert(result.raw.includes('fix sent for testing'), 'fix-queued is labelled in moderator language')
  ...
  assert(result.raw.includes('1 reply draft'), 'draft count mentioned')
  // Jargon checks — must NOT appear
  assert(!result.raw.includes('PR #'), 'no PR numbers leaked')
