...
| 4c | Head-meta e2e + hydration count | ⬜ | |
| 4c2 | Playwright root causes | 🔄 | 73✗ run1: (a) modtools-prod served STALE image-baked build calling PROD API (my local modtools build left modtools/.output in tree; .dockerignore root '.output' does NOT match modtools/.output; start script skips build if .output exists) → deleted artifacts, image rebuilt, container rebuilt with correct env, now bakes apiv2.localhost ✓; (b) new headless Chromium (PW 1.62) reports credentials NotSupportedError with NEW message → broadened existing fixtures allowlist entry. Rerun pending. |
