...
| T10 | Capacitor 8 + cap8 push fork | 🔄 | Folded into #1201 per Edward (as was #1204). Fork: `cap8` branch on capacitor-push-notifications-cap7 repo @ 139062f8a — upstream 8.1.2 delta applied ONTO the fork (drift was tiny: AGP 8.13/SDK 36/minSdk 24/firebase 25, type-preserving bundle.get, banner/list options, iOS 15 floor); package renamed @freegle/capacitor-push-notifications-cap8 v8.0.0. App: all @capacitor/* → ^8 (apple-sign-in stays 7.1.0, peers >=7.0.0 so cap8-compatible); stripe plugin 8.1.1 needs scoped override for its @stripe/stripe-js ^8.4 peer (app has ^9); stripeAndroidVersion 20.39→23.1 (plugin default); android project on cap8 template (compileSdk 36, keep minSdk 26, gradle 8.14.3); iOS floor 14→15 (Podfile + 4 pbxproj configs — REVIEWERS: drops iOS 14 installs, required by Capacitor 8); cap update regenerated plugin gradle/plugins.json (screen-orientation was missing, now in; dead InAppBrowser feature dropped — no code refs, allowlist excluded it, dep removed); bundledWebRuntime dead key removed from 3 configs; README-APP auth section refreshed (social-login reality, Yahoo gone). Native builds validate in CI only. |
