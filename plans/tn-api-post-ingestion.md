...
- **Unmoderated posters are not approved on arrival** — ✅ fixed 2026-08-21. `ingest()` used to map `DEFAULT`/`UNMODERATED` straight to `approved` (collection Approved + spatial index), while the email path had been changed to route them to Pending so `messages:contentcheck` gates them. That was drift, not a deliberate divergence, and it let a TN post a content rule would block go live unchecked. `ingest()` now mirrors the email path's `$awaitingContentCheck` branch exactly: Pending, **no** `notifyGroupMods`, **no** `addToSpatialIndex`, with the content check promoting clean posts within a minute. Because these posts frequently have no membership at all (section P), `ContentCheckService::isUserModerated()` now applies the same `'DEFAULT'` fallback for a message carrying a `tnpostid` — without it every non-member TN post would sit Pending forever, since a missing membership otherwise reads as moderated. Non-TN messages keep the old "no membership = moderated" reading.
...
- Tests, sharing the `CapturesRoutedLokiEntries` trait (which reads back what the real `LokiService` wrote to disk, never a mock):
  - `TnApiLokiParityTest::test_both_paths_produce_the_same_loki_entry_for_the_same_post_content` — **the test this section exists for.** Runs one TN post through BOTH real paths (`IncomingMailService::route()` and `PostSyncer::processPost()`) and diffs the two resulting Loki entries: identical labels but for `source`, identical key sets bar `tn_post_id`, and equal `routing_outcome`/`subject`/`group_id`/`group_name`/`user_id`. Fields that differ by design (`envelope_from`, `from_address`, `message_id`) are asserted explicitly so the divergence stays deliberate. The test plays `IncomingMailController`'s role by calling `logIncomingEmail()` itself after `route()` — no production code is modified, which is what makes this possible despite I.6.
    ...
    - Two fixture constraints, both documented in the test: the poster is **MODERATED**, not DEFAULT (kept now that the email-pends/API-approves divergence is fixed — see section D — because it pins the outcome on the posting status alone, so the Loki comparison doesn't depend on what the content check would make of the fixture's content; same reasoning as `EmailApiParityTest::seedParityUser()`); and each path gets its **own `post_id`** for the same content, because both paths synthesize the same `messages.messageid` and the API path's `tnpostid` idempotency check would otherwise see the email path's row and skip as a duplicate.
...
- Non-member TN posts are **not** specially forced to Pending — they follow the same routing tree as a `DEFAULT` member. This was an open question; resolved by defaulting to `'DEFAULT'` rather than special-casing non-members. Since the section-D fix, that tree lands every unmoderated poster (member or not) in Pending awaiting `messages:contentcheck`, which promotes it if clean — so the fallback now decides which *check* applies, not whether the post goes live unseen. `ContentCheckService::isUserModerated()` applies the same fallback for TN posts so a non-member's post is actually promotable there.
...
**Live-run findings, 2026-08-19 — clearing the noise before a 30-day sweep.** A one-hour window (`--date-min=2026-08-08T14:00:00Z --date-max=...T15:00:00Z`) reported `Layer 3 mismatches=3`, `Layer 5 mismatches=11`. Triaged, only one of the eleven was a real difference; the rest were tool artifacts that would have swamped a wide sweep:
  ...
  - ✅ **A title TN edited after emailing counted as a content mismatch.** Once the two items above were fixed, the entire remaining Layer 3/5 failure set on the re-run window was this one thing: `47116974` ("Various decorating paint" vs "Cupboard and wall paint") and `47116976` ("Table frame" vs "Table frame & glass top"). Neither is a Freegle bug — TN's *live* title matched what the API path had recorded, exactly, in both cases, and both posts' `expiration` arithmetic showed TN had mutated them since publication (`expiration` is pinned at original-publish + 90 days while `date` moves on a repost/edit, the same signal the Layer 1 date-bump check uses: `47116974` came back 89d, `47116976` 60d). The email path records the subject TN put in the email at send time; the API path records the title as it stands now; no care on our side can make those agree. `TNParityCheckCommand::reclassifySubjectMismatches()` now moves these into an informational section, mirroring `layer1Deleted`/`layer1BumpedOutOfWindow`/`layer1ResolvedOutcome`. Deliberately narrow, because a subject difference is also exactly what a real title-mangling bug (truncation, encoding) looks like: it applies only to post_ids whose *sole* disagreement on *every* layer that flagged them is the subject (`ParityComparer::subjectOnlyMismatchPostIds()`), and only when TN confirms the mutation — a null `expiration`, a lookup failure, or any additional mismatch keeps the post failing.
...
- ✅ `tests/Feature/TrashNothing/EmailApiParityTest.php` rewritten from the old byte-identical diff to 10 tests against `ParityComparer` (one "flags the issue" + one "silent when clean" per layer, the four silent cases sharing one `all_clean` fixture), using 5 new dedicated fixture pairs under `tests/fixtures/tn_sync/parity/{all_clean,layer1_missing,layer2_extra,layer3_mismatch,layer4_divergent_group}/`. Tests construct `EmailReplaySyncer`/`PostSyncer` directly (not via artisan) to pass the fixture-path overrides.
  - Found and fixed a real issue while building the `all_clean` fixture (verified via `artisan tinker`, not guesswork): a `DEFAULT`-status membership walked into a divergence — `IncomingMailService::handleGroupPost()` no longer auto-approves `DEFAULT` posters on arrival, while `GroupPostIngestionService::ingest()` still did — so even byte-identical content routed to different outcomes (`pending` vs `approved`). Switched the shared `seedParityUser()` helper to `MODERATED` (pends on both paths), matching the original test's approach, with the reasoning documented on the helper. **The divergence itself is now fixed** (section D): the API path pends unmoderated posters too, so `DEFAULT` would agree as well; `MODERATED` is kept only to keep the outcome independent of the content check.
...
1. **The skip predicate needed widening in what it *excludes*.** The design said "targetGroupName !== null && TN post header". That is wrong: `MailParserService::analyzeEnvelopeTo()` strips the `-volunteers`/`-auto` suffix and *still* reports a `targetGroupName`, so the predicate as designed would have swallowed volunteer mail, which `route()` handles in Phase 4 — before group posts. `isToVolunteers`/`isToAuto` are now excluded explicitly, with `isChatNotificationReply()` checked defensively alongside. The same argument applies to every *other* phase `route()` runs before Phase 5, so `claimedByAnEarlierPhase()` mirrors `route()` in its own order and also excludes `replyto-` addresses, bounces (both the DSN and the `bounce-` return-path forms), digest replies, dropped senders, auto-replies and self-sent mail — bounces and digest replies being the two with real side effects to lose. `isKnownSpammer()` is deliberately not mirrored: it is the only check needing database state, it ends in `DROPPED` so the email path writes nothing either way, and the API path ingests from TN regardless of what arrives by email. Covered by `TnEmailRoutingGateTest::test_ignores_volunteers_address_even_with_the_tn_header` and siblings.
...
3. **`unplaceable` is checked before the API lookup as well as after.** The S.4 table implies the API call comes first. Checking placement locally first is cheaper (a post that places nowhere needs no request against a 2-req/s limit) and safe *in one direction only*: a header naming coordinates outside every group's bounds is a post the email path would itself have dropped as an unknown group, so it cannot be evidence that the API path lost one, and crossposts always carry real coordinates so the local check cannot swallow one. Pinned by `CoverageVerifierTest::test_a_post_outside_every_group_boundary_is_expected_absent`, which asserts no lookup happens.

   **Corrected 2026-08-26**: the local check originally also short-circuited to `unplaceable` when the header was *missing*, and `ArchiveInventoryService` cast a malformed header to `(0.0, 0.0)` — a real point in the Gulf of Guinea that places in no group — so that landed in the same bucket. Either way a post whose header said nothing was filed as expected-absent without anyone asking TN where it actually is, which is precisely the Layer 1 coverage regression this command exists to catch, filed in the bucket nobody reads. Now: `parseCoordinates()` returns nulls for anything that is not a pair of numbers, a null header means "ask TN", and the placement verdict for every absentee that *is* looked up comes from the live post's own `latitude`/`longitude` (added to `lookupPostById()`'s return) — the coordinates `processPost()` would have placed it from. That also covers two cases the header could never have: a post TN never mapped (its coordinates are nullable), and one whose location was edited after the email was sent. Pinned by `test_a_missing_coordinates_header_is_judged_on_the_live_post_not_assumed_unplaceable`, `test_a_missing_coordinates_header_on_a_crosspost_copy_is_still_a_crosspost`, `test_a_post_the_live_api_never_mapped_is_expected_absent`, `test_a_post_whose_live_coordinates_moved_out_of_bounds_is_expected_absent` and `ArchiveInventoryServiceTest::test_a_malformed_coordinates_header_yields_no_coordinates_not_zero`.
...
**Unplaceable posts** need reconstructing rather than looking up. The API path drops posts with no coordinates or outside all group bounds (`PostSyncer::REASON_NO_COORDINATES` / `REASON_NOT_IN_ANY_GROUP_BOUNDS`). Post-cutover there is no email-path verdict to compare against — but running `Location::groupsNear()` on the same coordinates recovers the decision independently. Those coordinates are the **live post's** (`lookupPostById()` returns them), because they are what `processPost()` would have placed it from; the email's own `X-Trash-Nothing-Post-Coordinates` header is used only to skip the lookup when it *positively* resolves to no group. If the coordinates *do* resolve to a group and the post still isn't ingested, that is precisely the Layer 1 regression this whole check exists to catch, and it must never be filed as "expected absent" — and neither must a post whose header was absent or unparseable, which establishes nothing either way. See deviation 3 above.
