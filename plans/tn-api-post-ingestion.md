...
**Open follow-up (not yet addressed)**: now that production ingestion dedupes reposts itself, `ParityComparer::dedupeApiCrosspostsAndReposts()` may be partially redundant for cases the new `GroupPostIngestionService` logic already merges. It may still earn its keep for genuine same-run crossposts that land in *different* groups (each group gets its own message by design, so `findRepostCandidate()`'s per-group scope wouldn't merge them, but they may still represent the same real-world donation for parity-counting purposes) — not yet discussed with the user.
