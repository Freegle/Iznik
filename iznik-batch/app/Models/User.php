...
class User extends Model implements Auditable
{
    ...
    /**
     * SQL `NOT LIKE` patterns that exclude internal Freegle domains and
     * yahoogroups. Used by isInternalEmail() and the batch-join subquery so the
     * PHP filter (str_contains) and the SQL filter (NOT LIKE) stay in sync.
     */
    public static function internalEmailNotLikePatterns(): array
    {
        $patterns = [];
        foreach (config('freegle.mail.internal_domains', [
            'users.ilovefreegle.org',
            'groups.ilovefreegle.org',
            'direct.ilovefreegle.org',
            'republisher.freegle.in',
        ]) as $domain) {
            $patterns[] = '%@' . strtolower($domain);
        }
        foreach (config('freegle.mail.excluded_domain_patterns', ['@yahoogroups.']) as $p) {
            $patterns[] = '%' . strtolower($p) . '%';
        }
        return $patterns;
    }

    /**
     * Subquery: for each user, the best external email (highest preferred,
     * then highest validated). Excludes our own domains and yahoogroups so
     * batch senders never deliver to a users.ilovefreegle.org alias (which
     * loops back through IncomingMailService::handleDirectMail).
     *
     * Mirrors V1 User::getEmailPreferred() (iznik-server/include/user/User.php:467).
     *
     * Usage:
     *   ->leftJoinSub(User::externalEmailJoinSubquery(), 'ue', 'ue.userid', '=', 'users.id')
     *   ->whereNotNull('ue.email')
     *   ->addSelect('ue.email')
     */
    public static function externalEmailJoinSubquery(): \Illuminate\Database\Query\Builder
    {
        $patterns = self::internalEmailNotLikePatterns();

        $applyExclusion = function ($query, string $col) use ($patterns) {
            foreach ($patterns as $p) {
                $query->where($col, 'NOT LIKE', $p);
            }
        };

        return \Illuminate\Support\Facades\DB::table('users_emails as ue1')
            ->select('ue1.userid', 'ue1.email')
            ->where(function ($q) use ($applyExclusion) {
                $applyExclusion($q, 'ue1.email');
            })
            ->whereNotExists(function ($sq) use ($applyExclusion) {
                $sq->select(\Illuminate\Support\Facades\DB::raw(1))
                   ->from('users_emails as ue2')
                   ->whereColumn('ue2.userid', 'ue1.userid')
                   ->where(function ($q) use ($applyExclusion) {
                       $applyExclusion($q, 'ue2.email');
                   })
                   ->where(function ($cmp) {
                       $cmp->whereColumn('ue2.preferred', '>', 'ue1.preferred')
                           ->orWhere(function ($eq) {
                               $eq->whereColumn('ue2.preferred', '=', 'ue1.preferred')
                                  ->whereRaw('COALESCE(ue2.validated, "1970-01-01") > COALESCE(ue1.validated, "1970-01-01")');
                           })
                           ->orWhere(function ($eq2) {
                               $eq2->whereColumn('ue2.preferred', '=', 'ue1.preferred')
                                   ->whereRaw('COALESCE(ue2.validated, "1970-01-01") = COALESCE(ue1.validated, "1970-01-01")')
                                   ->whereColumn('ue2.id', '<', 'ue1.id');
                           });
                   });
            });
    }
