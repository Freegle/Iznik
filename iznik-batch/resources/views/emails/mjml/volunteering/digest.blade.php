<mjml>
  ...
  <mj-body background-color="#f4f4f4">
    ...
    @if (!empty($vol['photo_thumb']))
    {{-- Opportunity with photo: text left, image right --}}
    <mj-section background-color="#ffffff" padding="16px 20px 8px">
      <mj-column width="60%">
        ...
        @if (!empty($vol['online']))
        <mj-text font-size="13px" color="#555555" padding="0 0 4px">
          🌐 Online opportunity
        </mj-text>
        @elseif (!empty($vol['location']))
        <mj-text font-size="13px" color="#666666" padding="0 0 4px">
          📍 {{ $vol['location'] }}
        </mj-text>
        @endif
    ...
    @else
    {{-- Opportunity without photo: full width --}}
    <mj-section background-color="#ffffff" padding="16px 20px 8px">
      <mj-column>
        <mj-text font-size="18px" font-weight="bold" mj-class="text-success" padding="0 0 4px">
          <a href="{{ $vol['url'] }}" style="color: inherit; text-decoration: none;">{{ $vol['title'] }}</a>
    ...
    @if (!empty($vol['timecommitment']) || !empty($vol['applyby']))
    <mj-section background-color="#f9f9f9" padding="4px 20px 6px">
      <mj-column>
        @if (!empty($vol['timecommitment']))
        <mj-text font-size="12px" color="#555555" padding="0 0 2px">
          <strong>Time:</strong> {!! nl2br(e($vol['timecommitment'])) !!}
        </mj-text>
        @endif
        @if (!empty($vol['applyby']))
        <mj-text font-size="12px" color="#555555" padding="0">
          <strong>Apply by:</strong> {{ $vol['applyby'] }}
        </mj-text>
        @endif
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="#f9f9f9" padding="4px 20px 8px">
      <mj-column>
        <mj-text font-size="12px" color="#555555" padding="0">
          ...
          @if (!empty($vol['contactphone'])) &bull; <a href="tel:{{ $vol['contactphone'] }}">{{ $vol['contactphone'] }}</a>@endif
          @if (!empty($vol['contactemail'])) &bull; <a href="mailto:{{ $vol['contactemail'] }}">{{ $vol['contactemail'] }}</a>@endif
          @if (!empty($vol['contacturl'])) &bull; <a href="{{ $vol['contacturl'] }}">Website</a>@endif
        </mj-text>
      </mj-column>
    </mj-section>
    @endif
