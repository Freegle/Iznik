...
// The search branch of GetMemberships must honour the context cursor the same way the
// non-search branch does (TestGetMembershipsPagination above) - otherwise scrolling a
// search result list just re-runs the same top-N query and never advances, which is what
// made the client (which keys results by userid but sorts by first-insertion order) reshuffle
// already-shown members out of the visible window on scroll (Discourse 10001 post 10).
func TestGetMembershipsSearchPagination(t *testing.T) {
	...
	// No member should appear on both pages - if this fails, the search branch is ignoring
	// the context cursor and just re-returning the first page's results.
	page1IDs := map[uint64]bool{}
	for _, raw := range page1Members {
		m := raw.(map[string]interface{})
		page1IDs[uint64(m["id"].(float64))] = true
