...
func TestFilterReachBlocked(t *testing.T) {
	...
	// Self-sufficient: messages_reach belongs to PR A (merges before browse). Minimal
	...
	db.Exec("INSERT INTO messages_reach (msgid, lat, lng, polygon) VALUES (?, 51.5, -0.1, "+
	...
	db.Exec("INSERT INTO messages_reach (msgid, lat, lng, polygon) VALUES (?, 53.0, 2.0, "+
	...
	defer db.Exec("DELETE FROM messages_reach WHERE msgid IN (?, ?)", 970001, 970002)
