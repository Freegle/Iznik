...
// vertex of its reach polygon (as WKT, e.g. from ST_AsText(rr.polygon)).
// Mirrors UnifiedDigestService::reachRadiusMetres. Falls back to defaultM
// when the polygon can't be parsed or has no extent — defensive; every
// reach-arm post has a rippling_reach row with a real polygon.
