...
Measured on eight real isochrones (2026-08-25): 640 containment probes, 87 differences -
all in the boundary band, all at *exactly* 0.000m from the edge, all one direction
...
`ripple:verify-cells-parity` and read it, then the drop DDL node by node under RSU. The
...
`DROP COLUMN` rebuilds the table, which is what actually returns the space to the operating
system - so it is a long ALTER, and that is the point of the exercise.
