...
### 9b. Cell-set (raster) storage - `max_polygon_cells` (stacked on §9a)
...
overflow rings already use). Measured on a real production polygon: 45x smaller than the
stored WKT, with zero classification disagreement against the polygon-built form across
40,401 probe points.

Encoding is centralised in exactly one place - `iznik-spatial-go`'s
...
itself. Decoding is duplicated deliberately: a fixed, versioned, self-describing format
carries no such risk, so `App\Services\Ripple\CellSetService` (PHP) and
`iznik-server-go/rippling`'s own decode-only functions (Go; not a shared module - see
below) both parse it directly, proven identical via a golden vector generated by the real
encoder rather than hand-written.

`max_polygon_cells` (`MEDIUMBLOB NULL`, unindexed - nothing ever queries it in SQL) is the
first column converted: write-once, few readers, proves the whole pattern before `polygon`
(many more readers, plus the two in-place `ST_Difference` clips) or `overflow_bounds`
(different shape entirely). `MaxReachService::storeMaxPolygon` dual-writes it
best-effort; `isWithinMaxReach` and the Go `firstreply.ShouldPassThrough` read it first,
falling back to the §9a blob/hash test exactly as before if it is absent or fails to
decode. `ripple:backfill-max-reach-cells` fills existing rows whose eventual reach predates
this feature (the write path only ever touches a row once, when its eventual reach is first
computed).
...
carries its own small decode-only file; iznik-spatial-go - the only service that ever needed
the actual encoder - carries the whole `cellset` package as an ordinary internal package of
its own module.
