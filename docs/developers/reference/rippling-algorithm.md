...
overflow rings already use). Measured on one real production polygon: 45x smaller than the
...
to describe an area a grid describes cheaply. No real overflow ring has been through the
encoder yet, so the ring saving is inferred from the same boundary density (37,000
vertices a ring) rather than measured - run `ripple:backfill-ring-cells` against
production and measure the column to size it properly.
...
(nothing ever queries the bytes in SQL - they are opaque to MySQL and decoded in
application code), all additive: a deploy ahead of a backfill is a no-op because every
reader falls back to the geometry, or to its §9a hash.
...
**`outer_bound` and `inner_bound` are untouched by all of this** - still GEOMETRY, still
spatially indexed, still derived MySQL-side from the transient WKT in the same statement
as the polygon. The browse feed's own containment SQL (§7, `ReachBrowseWhere`) is
likewise unconverted: it is an expression inside a larger query whose plan hangs off that
index, and perturbing it is what caused the 2026-08-21 outage (§9a).
...
`overflow_bounds` remains the authority - the map overlay draws the ring vectors, the
"which lanes does this post carry" test is a `JSON_CONTAINS_PATH` against it, and
`has_overflow` (the generated, indexed flag the ring index build hangs off) is derived
from it.
