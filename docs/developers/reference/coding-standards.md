...
- **`Blocked` label is enforced, not advisory**: labelling a PR `Blocked` posts a failing `blocked-label` status, which is a required check on master, so the merge button greys out. Remove the label to clear it. A repo admin can still override the requirement deliberately, so this stops accidents, not decisions. See `.github/workflows/blocked-label-guard.yml`.
...
- Tools: `jscpd`, `golangci-lint`, PHPStan, ESLint.
