---
last_reviewed: 2026-09-02
owner: Freegle dev team
---

# Moderating Freegle: a guide for volunteers

Moderators are the volunteers who run local Freegle communities. You use **ModTools**
(modtools.org) to approve posts, look after members, handle spam and reports, and
configure your community.

This guide is organised around what you actually do:

1. **[Getting started](getting-started.md)** - getting into ModTools, roles and
   permissions, the dashboard, and switching between communities.
2. **[Moderating posts](moderating-posts.md)** - the pending queue, approving and
   rejecting, editing, why posts get held, standard messages, and reports.
3. **[Managing members](managing-members.md)** - the members list, reviews, spammers,
   chat review, notes, and banning or removing members.
4. **[Running your community](running-your-community.md)** - group settings, standard
   message sets, stories and newsletters, events, volunteering, broadcasts, logs and
   maps.

## First principles

A few things worth holding onto, whichever guide you are in:

- **Assume good faith.** Most members are kind and enthusiastic. Ban only with real
  evidence of harm, not because someone posts a lot or joins several communities.
- **Prefer editing over rejecting.** If a post breaks a small rule, edit it with a
  friendly note rather than turning it away. Reject only for the core rules: it must be
  **free** and **legal**.
- **Be personal, not corporate.** Members appreciate a friendly human reply, not a memo.
  Standard messages are a starting point, not a script.
- **Do not reject a post just for being "out of area".** With rippling out, posts travel
  between communities on purpose. See [moderating posts](moderating-posts.md).
- **Every community needs at least two owners.** If you are ever stuck, email the mentors
  at `mentors@ilovefreegle.org`, or use the volunteers' Discourse forum (the "Us" link in
  ModTools).

There is a moderator-facing guide to rippling out at
[./rippling-out.md](./rippling-out.md), and the
technical detail is in [../developers/reference/rippling-algorithm.md](../developers/reference/rippling-algorithm.md).

Council sponsorship deals - who sponsors us, what they are worth, and the quarterly
statistics councils receive - are covered in [./partnerships.md](./partnerships.md).

Post-moderation (auto-approve) - built but switched off - has its own reference
describing what it does, the case for and against, and what changes for moderators
and members: [./post-moderation.md](./post-moderation.md).
