...
class UnifiedDigestService
{
    ...
    protected function processGroupImmediate(object $cursorRow, bool $dryRun, ?int $userFilter = null): array
    {
        ...
        foreach ($messages as $message) {
            ...
            // Rippling reach gate (#0 step 4): once a post has a reach polygon, mail only the
            // members the ripple has actually reached, each at most once (messages_reach_notified
            // dedups across groups + successive expansions). No reach row → non-rippling post or
            // dark engine → mail every member, exactly as before (reachGatedRecipientIds → null).
            $msgid = (int) $message->mg_msgid;
            $reachRecipients = $this->reachGatedRecipientIds($msgid, array_keys($users->all()));

            $sponsorsCache = null;
            foreach ($users as $uid => $user) {
                if ($reachRecipients !== null && !isset($reachRecipients[(int) $uid])) {
                    continue; // the ripple hasn't reached this member yet (or already notified)
                }
                ...
                if (!$dryRun) {
                    ...
                    try {
                        ...
                        // Record the reach-gated send so a later expansion, or another group the
                        // post rippled into, never re-mails this member about the same post. Only
                        // on a successful spool — a failed send stays un-ledgered so it can retry.
                        if ($reachRecipients !== null) {
     ...
     */
    /**
     * Rippling reach gate for immediate mail (#0 step 4). Returns null when the post has no
     * reach polygon yet (non-rippling, or the engine is dark) — the caller then mails every
     * member as before. Otherwise returns a [userid => true] map of the members (from
     * $userIds) the reach now covers AND who have not already been notified about this post
     * (messages_reach_notified). A member's point follows the canonical resolution order:
     * settings.mylocation, else their lastlocation. Members with no resolvable location are
     * not covered (they wait until reach — and thus their group membership — grows).
     */
    private function reachGatedRecipientIds(int $msgid, array $userIds): ?array
    {
        if (empty($userIds)) {
            return [];
        }
        try {
            if (!DB::table('messages_reach')->where('msgid', $msgid)->exists()) {
                return null; // not a rippling post → ungated
            }
        } catch (\Throwable $e) {
            return null; // messages_reach absent (reach engine / PR A not deployed yet) → ungated
        }
        ...
        $rows = DB::select(
            "SELECT u.id FROM users u
             ...
             JOIN messages_reach mr ON mr.msgid = ?
             WHERE u.id IN ($placeholders)
               AND ST_Contains(
                     mr.polygon,
                     ST_SRID(POINT(
                       COALESCE(CAST(JSON_EXTRACT(JSON_EXTRACT(u.settings, '$.mylocation'), '$.lng') AS DECIMAL(10,6)), l.lng),
                       COALESCE(CAST(JSON_EXTRACT(JSON_EXTRACT(u.settings, '$.mylocation'), '$.lat') AS DECIMAL(10,6)), l.lat)
                     ), ?)
                   )
               AND NOT EXISTS (
                     SELECT 1 FROM messages_reach_notified n WHERE n.msgid = ? AND n.userid = u.id
                   )",
            array_merge([$msgid], $userIds, [$srid, $msgid])
        );
