...
# Allocate a swap file so a transient memory spike (Playwright workers + Laravel
# + Go all hitting peak together) doesn't OOM-kill the runner agent and produce
# an infrastructure_fail / heartbeat-timeout. fallocate is instant on ext4; if it
# fails (XFS without preallocation), fall back to dd.
# NOTE: this block runs inside the UNQUOTED SSHEOF heredoc, so any VM-side shell
# variable MUST be escaped as \$ (see start.sh below). The larger 20G swap is
# applied at job runtime by the orb's "Install system dependencies" step (a plain
# CircleCI run command, not a heredoc), which avoids the escaping pitfall.
