...
func main() {
	...
	api := buildPublicApp(srv)
	admin := buildAdminApp(srv)
...
// buildPublicApp creates the public-facing Fiber app (SPATIAL_PORT, default 8194).
// Registered routes: GET /health, GET /v1/:dataset/knn, POST /v1/:dataset/within_coords,
// and the Swagger/Redoc documentation UI at /swagger.
func buildPublicApp(srv *server) *fiber.App {
	// 256 KB read buffer to accommodate large polygon WKT query params.
	...
	// POST /v1/:dataset/within_coords — accepts polygon in request body to avoid URL
	// length limits for large isochrone polygons (e.g. 77-point WKT from routing server).
	...
	// Swagger UI (Redoc) — developer documentation; self-justifying, not an API endpoint.
...
// buildAdminApp creates the admin Fiber app (SPATIAL_ADMIN_PORT, default 8195).
// Admin routes mutate the spatial index and must not be exposed to the public network.
// Registered routes: POST /v1/:dataset/rebuild, POST /v1/:dataset/remove, POST /v1/:dataset/upsert.
func buildAdminApp(srv *server) *fiber.App {
	admin := fiber.New(fiber.Config{DisableStartupMessage: true})

	// POST /v1/:dataset/rebuild — triggers an async full rebuild of one dataset.
	...
	// POST /v1/:dataset/remove — incremental hard-delete of specific IDs.
	...
	// Request body: {"items": [{"id": int64, "wkt": string, "extra": object}]}
...
// Returns an error if the WKT is malformed, too large, or has too many vertices.
