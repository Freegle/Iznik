<template>
  <div class="member-chat">
    ...
    >
      <template #menu>
        <b-dropdown
          v-if="chat && !chat.systemchat"
          variant="link"
          no-caret
          toggle-class="shell-menu-btn"
          right
        >
    ...
    <ChatBlockModal
      ...
      :user="otheruser"
      @confirm="block"
      @hidden="showBlock = false"
    />
    <ChatReportModal
      v-if="showReport"
      :id="id"
      :user="otheruser"
      @hidden="showReport = false"
    />
    <ProfileModal
...
const otheruser = computed(() =>
  chat.value?.otheruid ? userStore.byId(chat.value.otheruid) : null
)
...
const subtitle = computed(() => {
  ...
  if (miles !== undefined && miles !== null)
    return `About ${Math.round(miles)} mile${Math.round(miles) === 1 ? '' : 's'} away`
  return null
})
