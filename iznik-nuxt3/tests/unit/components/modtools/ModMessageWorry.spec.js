...
describe('ModMessageWorry', () => {
  ...
  describe('IpAbuse check', () => {
    it('shows a plain, friendly heading for the shared-connection case (not "IP abuse")', () => {
      const wrapper = mountWithReasons([
        {
          check: 'IpAbuse',
          detail: 'IP 104.28.40.129 recently used by 8 different user accounts',
          userCount: 8,
          ip: '104.28.40.129',
          users: [44997601, 44987978, 39329507],
        },
      ])
      const text = wrapper.text()
      expect(text).toContain('Posted from a much-used connection')
      expect(text).not.toContain('IP abuse')
      // Uses the count, and reassures that it is often innocent + says what to do.
      expect(text).toContain('8 different accounts')
      expect(text).toContain('often completely innocent')
      expect(text).toContain("it's fine to approve")
    })

    it('keeps the IP and user IDs visible for reference', () => {
      const wrapper = mountWithReasons([
        {
          check: 'IpAbuse',
          detail: 'IP 104.28.40.129 recently used by 8 different user accounts',
      ...
      const text = wrapper.text()
      expect(text).toContain('104.28.40.129')
      expect(text).toContain('44997601')
    })

    it('falls back to users.length when userCount is absent (older stored flags)', () => {
      const wrapper = mountWithReasons([
        {
          check: 'IpAbuse',
          detail: 'IP 10.0.0.1 recently used by 6 different user accounts',
          ip: '10.0.0.1',
          users: [1, 2, 3, 4, 5, 6],
        },
      ])
      expect(wrapper.text()).toContain('6 different accounts')
    })

    it('shows the many-communities wording for the group-count case', () => {
      const wrapper = mountWithReasons([
        {
          check: 'IpAbuse',
          detail: 'IP 10.0.0.2 recently used to post to 24 different groups',
          groupCount: 24,
          ip: '10.0.0.2',
          groups: [1, 2, 3],
        },
      ...
      const text = wrapper.text()
      expect(text).toContain('Posted to many communities')
      expect(text).toContain('24 different communities')
      expect(text).not.toContain('IP abuse')
