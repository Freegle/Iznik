...
describe('MyPostsPostsList', () => {
  ...
  describe('old posts separation (issue 9481)', () => {
    it('shows only old posts when old-posts toggle is active, not active posts too', async () => {
      // Bug: toggling "Show x old posts" used posts.value (all posts),
      // so active posts were interspersed with old ones by date.
      // Fix: use oldPosts.value so the list shows ONLY old posts,
      // matching the Chats "Show hidden chats" behaviour.
      const wrapper = createWrapper({
        loading: false,
        posts: [
          { id: 1, hasoutcome: false, arrival: '2024-01-03' }, // active
          { id: 2, hasoutcome: true, arrival: '2024-01-02' },  // old
          { id: 3, hasoutcome: true, arrival: '2024-01-01' },  // old
        ],
        show: 10,
      })
      ...
      const shownIds = wrapper
        .findAll('.my-message')
        .map((m) => m.attributes('data-id'))

      // Only the two old posts should be visible.
      expect(shownIds).toHaveLength(2)
      expect(shownIds).toContain('2')
      expect(shownIds).toContain('3')
      // The active post must NOT appear alongside old posts.
      expect(shownIds).not.toContain('1')
    })

    it('shows only active posts by default (before toggle)', () => {
      const wrapper = createWrapper({
        loading: false,
        posts: [
          { id: 1, hasoutcome: false, arrival: '2024-01-03' }, // active
      ...
      // By default only the active post is shown.
      expect(shownIds).toHaveLength(1)
      expect(shownIds).toContain('1')
      expect(shownIds).not.toContain('2')
    })
  })
})
