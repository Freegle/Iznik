...
vi.mock('~/stores/message', () => ({
  useMessageStore: () => ({
    byId: (id) => (id === 1 ? { id: 1, type: 'Offer', subject: 'OFFER: Grey sofa (EH3)', textbody: 'Three seater', attachments: [], area: { name: 'Edinburgh' } } : null),
  }),
}))
...
describe('PostCard', () => {
  it('shows the cleaned title, type, area and distance, and emits reply and expand', async () => {
    const w = mount(PostCard, { props: { id: 1, miles: 2.4 }, global: { stubs } })
  ...
  it('shows the body when expanded and renders nothing for an unknown post', () => {
    const w = mount(PostCard, { props: { id: 1, expanded: true }, global: { stubs } })
