...
const stubs = { ProfileImage: true, ProxyImage: true, OurUploadedImage: true, 'b-dropdown': { template: '<div><slot name="button-content" /><slot /></div>' }, 'b-dropdown-item': { template: '<button @click="$emit(\'click\')"><slot /></button>' } }
const item = { id: 3, userid: 7, displayname: 'Sam', message: 'Anyone know a good plumber?', timestamp: '2026-09-08T09:00:00Z', loves: 2, loved: false }
...
describe('ChitChatBubble', () => {
  ...
  it('a reply quotes what it answers and tapping the quote jumps; my own bubbles sit right without a name', async () => {
    ...
    const w = mount(ChitChatBubble, { props: { item: { ...item, id: 4 }, quote, mine: true }, global: { stubs } })
