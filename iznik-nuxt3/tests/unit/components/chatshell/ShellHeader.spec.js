...
describe('ShellHeader', () => {
  it('shows title, subtitle, initial avatar and a back link with an unread badge', () => {
    const w = mount(ShellHeader, {
      props: {
        title: 'Freegle',
        subtitle: 'Give and get',
        back: '/chats',
        badge: 3,
      },
      global: { stubs },
    ...
    expect(w.find('[data-testid="shell-back"]').attributes('href')).toBe(
      '/chats'
    )
  ...
  it('has no back link without a target and caps the badge', () => {
    const w = mount(ShellHeader, {
      props: { title: 'X', back: '/chats', badge: 250 },
      global: { stubs },
    })
    expect(w.find('[data-testid="shell-badge"]').text()).toBe('99+')
    const none = mount(ShellHeader, {
      props: { title: 'X' },
      global: { stubs },
    })
