...
const mockMessage = {
  ...
  bulkcount: 1,
  bulkitems: [
    { id: 10, position: 0, name: 'Cabinet', quantity: 2, interest: [] },
  ],
}
...
vi.mock('~/components/HelperProposalCard', () => ({
  default: {
    ...
    template: '<div class="proposal-stub" />',
  },
}))
...
vi.mock('#imports', () => ({
  ...
  useRuntimeConfig: () => ({
    public: { USER_SITE: 'https://www.ilovefreegle.org' },
  }),
...
describe('ClearanceManager share update link', () => {
  ...
  it('offers a "Get an update link" button to the owner', () => {
    ...
    expect(w.find('[data-testid="clearance-sharelink-get"]').exists()).toBe(
      true
    )
  })
  ...
  it('mints a link and shows a shareable /clearance/update URL', async () => {
    ...
    expect(w.find('[data-testid="clearance-sharelink-input"]').exists()).toBe(
      true
    )
  })
})
