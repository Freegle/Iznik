...
describe('yourposts', () => {
  ...
  it('scores repliers transparently and orders them, ties in reply order', () => {
    ...
    const near = {
      userid: 2,
      date: hours(5),
      snippet: 'I could collect tomorrow evening from you',
      miles: 1,
      ratings: { Up: 5, Down: 0 },
    }
    ...
    const tie = orderRepliers(
      [
        { userid: 7, date: hours(30) },
        { userid: 8, date: hours(31) },
      ],
      post
    )
    expect(tie.map((r) => r.userid)).toEqual([7, 8])
    expect(
      scoreReplier({ userid: 9, date: hours(1), myRatingDown: true }, post)
        .score
    ).toBeLessThan(0)
  })

  it('allocates a split from what people asked for, never over the pool', () => {
    const alloc = allocate(4, [
      { userid: 1, wanted: 3 },
      { userid: 2 },
      { userid: 3, wanted: 2 },
    ])
    ...
    expect(
      remaining({ availablenow: 4, promises: [{ userid: 1, count: 3 }] })
    ).toBe(1)
  ...
  it('tells the story of a post from its data, oldest first', () => {
    const post = {
      id: 5,
      type: 'Offer',
      subject: 'OFFER: Four chairs (EH3)',
      arrival: hours(0),
      availablenow: 4,
      promises: [{ userid: 2, promisedat: hours(6), count: 2 }],
      outcomes: [],
      heldreplies: 1,
    }
    const replies = [
      ...
      },
      {
        ...
        date: hours(2),
        chatid: 12,
        snippet: 'Could I have two please',
      },
    ]
    const trysts = [
      { id: 1, msgid: 5, user1: 9, user2: 2, arrangedfor: hours(7) },
    ]
    ...
    expect(ev[5].chips.map((c) => c.value)).toEqual([
      'taken:5:2',
      'notyet:5:2',
      'noshow:5:2',
    ])
  ...
  it('says a post has gone quiet only after three days with no replies and none held', () => {
    ...
    const soon = eventsFor(post, {
      replies: [],
      trysts: [],
      now: T0 + 2 * 24 * 3600000,
    })
    expect(soon.map((e) => e.kind)).toEqual(['posted'])
    const later = eventsFor(post, {
      replies: [],
      trysts: [],
      now: T0 + 4 * 24 * 3600000,
    })
    expect(later.map((e) => e.kind)).toEqual(['posted', 'quiet'])
    const held = eventsFor(
      { ...post, heldreplies: 1 },
      { replies: [], trysts: [], now: T0 + 4 * 24 * 3600000 }
    )
  ...
  it('collapses history before a repost and counts unread against a watermark', () => {
    ...
    const replies = [
      { userid: 1, displayname: 'Old', date: hours(5), chatid: 1 },
      { userid: 2, displayname: 'New', date: hours(50), chatid: 2 },
    ]
