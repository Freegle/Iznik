...
import { parseSse } from '~/api/AssistantAPI'
import AssistantAPI, { ANON_KEY } from '~/api/AssistantAPI'

vi.mock('~/stores/auth', () => ({ useAuthStore: () => ({ auth: { jwt: 'jwt1', persistent: null } }) }))
...
describe('AssistantAPI', () => {
  ...
  it('streams deltas, stores the anonymous token and returns the turn', async () => {
    const api = new AssistantAPI({ APIv2: 'http://api' })
    const body = 'event: identity\ndata: {"anon":"tok.sig"}\n\nevent: delta\ndata: {"text":"Hel"}\n\nevent: delta\ndata: {"text":"lo"}\n\nevent: turn\ndata: {"conversation":"c1","say":"Hello","state":"HUB"}\n\n'
    global.fetch = vi.fn(async (url, opts) => {
      expect(url).toBe('http://api/assistant/turn')
      expect(opts.headers.Authorization).toBe('"jwt1"')
      return { ok: true, headers: { get: () => 'tok.sig' }, body: streamOf(body) }
    })
    const deltas = []
    const turn = await api.turn({ tap: 'give' }, { onDelta: (t) => deltas.push(t) })
  ...
  it('throws on an error event or a bad status', async () => {
    const api = new AssistantAPI({ APIv2: 'http://api' })
    global.fetch = vi.fn(async () => ({ ok: true, headers: { get: () => null }, body: streamOf('event: error\ndata: {"message":"nope"}\n\n') }))
    await expect(api.turn({ text: 'x' })).rejects.toThrow('nope')
    global.fetch = vi.fn(async () => ({ ok: false, status: 500, headers: { get: () => null }, body: null }))
