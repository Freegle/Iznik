/**
 * The support agent's system prompt: who it is talking to, the trust boundary,
 * the tools, the investigation playbook and the house style.
 *
 * Kept in its own module with no requires so that CI's bare `node --test` (no
 * node_modules) can check it. support-agent.js pulls in tools.js and with it
 * mysql2, so nothing that lives there can be unit-tested that way.
 */

function systemPrompt(userId, codebase) {
  const member = userId
  ...
  return (
    ...
    `- **Suggested replies to the member**: when you draft words for the volunteer to send on, write them exactly as ` +
    `they will be sent. Address the member directly in the second person ("you haven't verified your email yet"), ` +
    `never the third person ("she hasn't"), and use no internal names (tables, files, tools). Put the draft under a ` +
    `**Suggested reply** heading, in a blockquote, so it can be copied out as-is, and keep the analysis for the ` +
    `volunteer outside it.\n` +
